...
- `--mangled-kernel` is the exact raw selector used by adapters when they enumerate kernel IPs from symbols
...
./build/dwarf-parser-check \
  ...
  --mangled-kernel _ZTSN12_GLOBAL__N_117PrimaryGEMMKernelE \
  --adapters rust-gimli \
  --all-ips
```
...
The versioned sample file is the standalone ELF/DWARF module generated by `data_generation`:

```text
dwarf_files/PrimaryGEMMKernel.dwarf
```

Useful names observed in that file:

- display kernel name: `(anonymous namespace)::PrimaryGEMMKernel`
- kernel selector: `_ZTSN12_GLOBAL__N_117PrimaryGEMMKernelE`
- computation-body symbol: `_ZN12_GLOBAL__N_14GEMMEPKfS1_PfjN4sycl3_V12idILi2EEE`

Use the kernel selector with `--all-ips`:
